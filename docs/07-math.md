...
反引号包裹的内容也可以放入美元公式中，用于避免代码字符被当作数学语法的一部分：
...
GitHub也支持使用 `math` 作为围栏代码块的语言标识符：
...
在GitHub中，`math` 围栏会按数学公式处理；普通Markdown渲染器可能只把它当作带有 `math` 语言标识符的代码块。
`````
