...
- 明确GitHub网页端为主要展示目标，并区分平台功能、独立HTML、完整LaTeX和第三方扩展。

### 修复

- 清理拆分文档中的无意义行尾空白。
- 恢复图片章节和GitHub组件章节在首次拆分时错位的内容。
- 修正`[TOC]`、`flow`、`diff`、徽章以及HTML音视频示例的GitHub兼容性说明。
- 移除失效的Star History匿名动态图地址，改用页面链接和可直接显示的仓库内静态SVG方案。
- 修复本地链接检查测试在Windows和Linux上结果不一致的问题，并关闭无模块项目的Go依赖缓存。
